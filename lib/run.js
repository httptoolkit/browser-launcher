...
function formatNoProxy(options, browser) {
...
setups.firefox = function (browser, options, callback) {
  ...
  if (options.proxy) {
    ...
    assign(prefs, {
      ...
      'network.proxy.no_proxies_on': formatNoProxy(options, 'firefox')
    });
  }
...
setups.chrome = function (browser, options, callback) {
  ...
  if (options.proxy) {
	...
	var noProxy = formatNoProxy(options, 'chrome')
	if (noProxy) {
      ...
      setups[browser.type](browser, options, function (err, args, defaultArgs) {
        ...
        if (!options.skipDefaults) {
          args = args.concat(defaultArgs);
				...
				var noProxy = formatNoProxy(options, 'env');
