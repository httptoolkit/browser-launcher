var headless = require( 'headless' ),
	fs = require( 'fs' ),
	path = require( 'path' ),
	_ = require( 'lodash' ),
	Instance = require( './instance' ),
	argue = {};

argue.firefox = function( br, uri, opts, cb ) {
	var file = path.join( path.dirname( br.profile.file ), 'user.js' ),
		prefs = {
			'browser.shell.checkDefaultBrowser': false,
			'browser.bookmarks.restore_default_bookmarks': false,
			'dom.disable_open_during_load': false,
			'dom.max_script_run_time': 0
		};

	opts.options = opts.options || [];

	if ( opts.proxy ) {
		var m = /^(?:http:\/\/)?([^:\/]+)(?::(\d+))?/.exec( opts.proxy ),
			host = JSON.stringify( m[ 1 ] ),
			port = m[ 2 ] || 80;

		_.extend( prefs, {
			'network.proxy.http': host,
			'network.proxy.http_port': +port,
			'network.proxy.type': 1,
			'browser.cache.disk.capacity': 0,
			'browser.cache.disk.smart_size.enabled': false,
			'browser.cache.disk.smart_size.first_run': false,
			'browser.sessionstore.resume_from_crash': false,
			'browser.startup.page': 0,
			'network.proxy.no_proxies_on': JSON.stringify( opts.noProxy || '' )
		} );
	}

	if ( opts.prefs ) {
		_.extend( prefs, opts.prefs );
	}

	prefs = Object.keys( prefs ).map( function( name ) {
		return 'user_pref("' + name + '", ' + prefs[ name ] + ');';
	} ).join( '\n' );

	fs.writeFile( file, prefs, function( err ) {
		if ( err ) {
			cb( err );
		} else {
			cb( null, opts.options.concat( [
				'--no-remote',
				'-P', br.profile.name,
				uri
			] ) );
		}
	} );
};

argue.ie = function( br, uri, opts, cb ) {
	cb( null, [ uri ] );
};

argue.safari = function( br, uri, opts, cb ) {
	cb( null, [ uri ] );
};

argue.chrome = function( br, uri, opts, cb ) {
	opts.options = opts.options || [];

	cb( null, opts.options.concat( [
		opts.proxy ? '--proxy-server=' + opts.proxy : null,
		br.profile ? '--user-data-dir=' + br.profile : null,
		'--disable-restore-session-state',
		'--no-default-browser-check',
		'--disable-popup-blocking',
		'--disable-translate',
		'--start-maximized',
		'--disable-default-apps',
		'--disable-sync',
		'--enable-fixed-layout',
		'--no-first-run',
		'--noerrdialogs ',
		uri
	] ).filter( Boolean ) );
};

argue.phantom = function( br, uri, opts, cb ) {
	var file = path.join( __dirname, '../res/phantom.js' );

	opts.options = opts.options || [];

	cb( null, opts.options.concat( [
		opts.proxy ? '--proxy=' + opts.proxy.replace( /^http:\/\//, '' ) : null,
		file,
		uri
	] ).filter( Boolean ) );
};

function copy( src, dest, cb ) {
	var rs = fs.createReadStream( src ),
		ws = fs.createWriteStream( dest ),
		called = false;

	function done( err ) {
		if ( !called ) {
			called = true;
			cb( err );
		}
	}

	rs.on( 'error', done );
	ws.on( 'error', done );
	ws.on( 'close', function() {
		done();
	} );

	rs.pipe( ws );
}

argue.opera = function( br, uri, opts, cb ) {
	var prefs = {
			old: 'operaprefs.ini',
			blink: 'Preferences'
		},
		options = {
			old: [
				// br.profile ? '-pd ' + br.profile : null, // disabled temporarily
				'-nosession',
				'-nomail',
				uri
			],
			// using the same rules as for chrome
			blink: [
				br.profile ? '--user-data-dir=' + br.profile : null,
				'--disable-restore-session-state',
				'--no-default-browser-check',
				'--disable-popup-blocking',
				'--disable-translate',
				'--start-maximized',
				'--disable-default-apps',
				'--disable-sync',
				'--enable-fixed-layout',
				'--no-first-run',
				'--noerrdialogs',
				uri
			]
		},
		prefFile = prefs[ major( br.version ) >= 15 ? 'blink' : 'old' ],
		src = path.join( __dirname, '../res/' + prefFile ),
		dest = path.join( br.profile, prefFile );

	opts.options = opts.options || [];

	copy( src, dest, function( err ) {
		if ( err ) {
			cb( err );
		} else {
			cb(
				null,
				opts.options.concat( options[ major( br.version ) >= 15 ? 'blink' : 'old' ] ).filter( Boolean )
			);
		}
	} );
};

module.exports = function( config, name, version ) {
	var browser = selectMatch( config, name, version );

	if ( !browser ) {
		return;
	}

	return function( uri, opts, cb ) {
		if ( opts.headless && !browser.headless ) {
			headless( function( err, proc, display ) {
				if ( err ) {
					return cb( err );
				}

				run( {
					DISPLAY: ':' + display
				} );
			} );
		} else {
			run( {} );
		}

		function run( env_ ) {
			var env = {},
				cwd = process.cwd();

			Object.keys( process.env ).forEach( function( key ) {
				env[ key ] = process.env[ key ];
			} );

			Object.keys( env_ ).forEach( function( key ) {
				env[ key ] = env_[ key ];
			} );

			argue[ browser.type ]( browser, uri, opts, function( err, args ) {
				if ( err ) {
					return cb( err );
				}

				if ( opts.noProxy && env.no_proxy === undefined ) {
					env.no_proxy = opts.noProxy;
				}

				if ( opts.proxy && env.http_proxy === undefined ) {
					env.http_proxy = opts.proxy;
				}

				if ( opts.proxy && env.HTTP_PROXY === undefined ) {
					env.HTTP_PROXY = opts.proxy;
				}

				switch ( process.platform ) {
					case 'win32':
						browser.command = browser.command.replace( /"/g, '' ); //Ensure all the quotes are removed
						cwd = require( 'path' ).dirname( browser.command ); //Change directory to the app's base (Chrome)
						break;
					case 'darwin':
						if ( browser.name !== 'firefox' && browser.name !== 'phantomjs' ) { //Use the bin path under the hood
							// creates: open --wait-apps --new --fresh -a /Path/To/Executable <url> --args <rest of app args>
							args.unshift( '--wait-apps', '--new', '--fresh', '-a', browser.command, args.pop(), '--args' );

							browser.processName = browser.command;
							browser.command = 'open';
						}
						break;
				}

				cb( null, new Instance( _.extend( {}, browser, {
					args: args,
					env: env,
					cwd: cwd
				} ) ) );
			} );
		}
	};
};

function major( version ) {
	return +version.split( '.' )[ 0 ];
}

function selectMatch( config, name, version ) {
	var order = ( config.preference || [] ).concat( Object.keys( config.browsers ) ),
		matching,
		browsers,
		i;

	function filter( browsers ) {
		return browsers.filter( function( b ) {
			return b.name === name && matches( b.version, version );
		} ).sort( function( a, b ) {
			return major( b.version ) - major( a.version );
		} );
	}

	for ( i = 0; i < order.length; i++ ) {
		browsers = config.browsers[ order[ i ] ];

		matching = filter( browsers );

		if ( matching.length ) {
			return matching[ 0 ];
		}
	}
}

function matches( version, pattern ) {
	if ( pattern === undefined || pattern === '*' ) {
		return true;
	}

	// todo: real semvers
	var vs = version.split( '.' ),
		ps = pattern.split( '.' ),
		i;

	for ( i = 0; i < ps.length; i++ ) {
		if ( ps[ i ] === 'x' || ps[ i ] === '*' ) {
			continue;
		}

		if ( ps[ i ] !== vs[ i ] ) {
			return false;
		}
	}

	return true;
}
