	...
	Object.keys(browsers).forEach(function(type) {
		...
		Object.keys(variants).map(function(name) {
			if (shouldUseDarwinDetector(name)) {
				return flatBrowsers.push(assign({type, name}, config)); // `command` not needed, will use darwin detector
			}

			return variants[name].map(function(command) {
				flatBrowsers.push(assign({type, name, command}, config));
			});
		});
	});
			...
			if (!err) {
				if (Array.isArray(version)) {
					version.forEach(function(item) {
						flatBrowser.command = item.path || flatBrowser.command;
						flatBrowser.version = item.version;
						available.push(flatBrowser);
					});
				} else {
					flatBrowser.command = path || flatBrowser.command;
					flatBrowser.version = version;
					available.push(flatBrowser);
				}
			}
