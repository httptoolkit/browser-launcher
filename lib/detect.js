 ...
 * @param {Function} callback Callback function
 */
function runBrowserCommand( name, regex, callback ) {
...
function checkUnix(commands, regex, cb) {
	...
	commands.forEach(function(command) {
		...
		/*
		 ...
		 successful command invokation, because then it will be called more than `browserPlatforms.length` times.
		 ...
		 passed on
		 */
		runBrowserCommand(command, regex, function linuxDone(err, version) {
			checkCount++;
			if (!err) {
